...
int ksw_gg(void *km, int qlen, const uint8_t *query, int tlen,
  ...
  for (i = 0; i < tlen; ++i) {  // target sequence is in the outer loop
    ...
    if (m_cigar_ && n_cigar_ && cigar_) {
      ...
      off[i] = st;
      for (j = st; j < en; ++j) {
        // At the beginning of the loop: eh[j] = { H(i-1,j-1), E(i,j) }, f =
        // F(i,j) and h1 = H(i,j-1) Cells are computed in the following order:
  ...
  kfree(km, eh);
  if (m_cigar_ && n_cigar_ && cigar_) {
    ksw_backtrack(km, 0, 0, 0, z, off, 0, n_col, tlen - 1, qlen - 1, m_cigar_,
                  n_cigar_, cigar_);
    kfree(km, z);
    kfree(km, off);
  }
