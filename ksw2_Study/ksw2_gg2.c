...
int ksw_gg2(void *km, int qlen, const uint8_t *query, int tlen,
            ...
            int8_t e, int w, int *m_cigar_, int *n_cigar_, uint32_t **cigar_) {
  ...
  for (r = 0; r < qlen + tlen - 1; ++r) {
    ...
    if (st != 0) {
      if (r > st + st + w - 1)
        ...
        x1 = x[st - 1], v1 = v[st - 1];  // (r-1, st-1) in the band
    } else
      x1 = 0, v1 = r ? q : 0;
    if (en != r) {
      if (r < en + en - w - 1)
        y[en] = u[en] =
            0;  // (r-1,en) out of the band; TODO: is this line necessary?
    } else
      y[r] = 0, u[r] = r ? q : 0;
    // loop fission: set scores first
    for (t = st; t <= en; ++t) s[t] = mat[target[t] * m + qr[t + qlen - 1 - r]];
    // core loop
    if (m_cigar_ && n_cigar_ && cigar_) {
      uint8_t *pr = p + r * n_col;
      off[r] = st;
      for (t = st; t <= en; ++t) {
        /* At the beginning of the loop, v1=v(r-1,t-1), x1=x(r-1,t-1),
           u[t]=u(r-1,t), v[t]=v(r-1,t), x[t]=x(r-1,t), y[t]=y(r-1,t) a      =
           x(r-1,t-1) + v(r-1,t-1) b      = y(r-1,t)   + u(r-1,t) z      = max{
           S(t,r-t) + 2q + 2r, a, b } u(r,t) = z - v(r-1,t-1) v(r,t) = z -
           u(r-1,t) x(r,t) = max{ 0, a - z + q } y(r,t) = max{ 0, b - z + q }
         */
        uint8_t d;
        int8_t u1;
        int8_t z = s[t] + qe2;
    ...
    if (r > 0) {
      if (last_H0_t >= st && last_H0_t <= en)
        ...
        ++last_H0_t, H0 += u[last_H0_t] - qe;
    } else
      H0 = v[0] - qe - qe, last_H0_t = 0;
  }
  kfree(km, u);
  kfree(km, v);
  kfree(km, x);
  ...
  kfree(km, qr);
  if (m_cigar_ && n_cigar_ && cigar_) {
    ksw_backtrack(km, 1, 0, 0, p, off, 0, n_col, tlen - 1, qlen - 1, m_cigar_,
                  n_cigar_, cigar_);
    kfree(km, p);
    kfree(km, off);
  }
