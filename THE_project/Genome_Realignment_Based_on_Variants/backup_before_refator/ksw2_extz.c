...
void ksw_extz(void *km, int qlen, const uint8_t *query, int tlen,
              ...
              int8_t gape, int w, int zdrop, int flag, ksw_extz_t *ez) {
  eh_t *eh;
  int8_t *qp;  // query profile
  int32_t i, j, k, max_j = 0, gapoe = gapo + gape, n_col, *off = 0,
                   with_cigar = !(flag & KSW_EZ_SCORE_ONLY);
  uint8_t *z =
      0;  // backtrack matrix; in each cell: f<<4|e<<2|h; in principle, we can
          // halve the memory, but backtrack will be more complex
  ...
  for (; j <= qlen; ++j)
    eh[j].h = eh[j].e = KSW_NEG_INF;  // everything is -inf outside the band
  ...
  for (i = 0; i < tlen; ++i) {  // target sequence is in the outer loop
    ...
    if (!with_cigar) {
      ...
      for (j = st; j <= en; ++j) {
        ...
        p->e = e;
        f -= gape;
        d |= f > h ? 0x10 : 0;  // if we want to halve the memory, use one bit
                                // only, instead of two
        f = f > h ? f : h;
        zi[j - st] =
            d;  // z[i,j] keeps h for the current cell and e/f for the next cell
      }
    } else {
      uint8_t *zi = &z[(long)i * n_col];
      off[i] = st;
    ...
    if (i == tlen - 1) ez->mte = max, ez->mte_q = max_j;
    if (ksw_apply_zdrop(ez, 0, max, i, max_j, zdrop, gape)) break;
    if (i == tlen - 1 && en == qlen - 1) ez->score = eh[qlen].h;
  }
  kfree(km, qp);
  kfree(km, eh);
  if (with_cigar) {
    ...
    if (!ez->zdropped && !(flag & KSW_EZ_EXTZ_ONLY))
      ksw_backtrack(km, 0, rev_cigar, 0, z, off, 0, n_col, tlen - 1, qlen - 1,
                    &ez->m_cigar, &ez->n_cigar, &ez->cigar);
    else if (ez->max_t >= 0 && ez->max_q >= 0)
      ksw_backtrack(km, 0, rev_cigar, 0, z, off, 0, n_col, ez->max_t, ez->max_q,
                    &ez->m_cigar, &ez->n_cigar, &ez->cigar);
    kfree(km, z);
    kfree(km, off);
  }
