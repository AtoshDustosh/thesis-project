...
static Position FindSibling(Position Parent, int i) {
  ...
  Sibling = NULL;
  if (i == 0) {
    if (Parent->Children[1]->KeyNum < Limit) Sibling = Parent->Children[1];
...
static Position InsertElement(int isKey, Position Parent, Position X,
  ...
  if (isKey) {
    ...
    if (Parent != NULL) Parent->Key[i] = X->Key[0];
    ...
    if (X->Children[0] == NULL) {
      if (i > 0) Parent->Children[i - 1]->Next = X;
...
static Position RemoveElement(int isKey, Position Parent, Position X, int i,
                              int j) {
  int k, Limit;
...
static Position MoveElement(Position Src, Position Dst, Position Parent, int i,
                            int n) {
  KeyType TmpKey;
  Position Child;
  int j, SrcInFront;
  ...
  if (Src->Key[0] < Dst->Key[0]) SrcInFront = 1;
  ...
  if (SrcInFront) {
    ...
    /* 将树叶节点重新连接 */
    if (Src->KeyNum > 0) FindMostRight(Src)->Next = FindMostLeft(Dst);
    ...
    if (Src->KeyNum > 0) FindMostRight(Dst)->Next = FindMostLeft(Src);
  }
...
static BPlusTree RecursiveInsert(BPlusTree T, KeyType Key, int i,
                                 BPlusTree Parent) {
  int j, Limit;
  Position Sibling;
  ...
  while (j < T->KeyNum && Key >= T->Key[j]) {
    /* 重复值不插入 */
    if (Key == T->Key[j]) return T;
    j++;
  }
  if (j != 0 && T->Children[0] != NULL) j--;

  /* 树叶 */
  if (T->Children[0] == NULL) T = InsertElement(1, Parent, T, Key, i, j);
  /* 内部节点 */
  else
  ...
  if (Parent != NULL) Parent->Key[i] = T->Key[0];
...
static BPlusTree RecursiveRemove(BPlusTree T, KeyType Key, int i,
  ...
  } else if (j == T->KeyNum || Key < T->Key[j])
    j--;
  ...
  if (Parent == NULL && T->Children[0] != NULL && T->KeyNum < 2) NeedAdjust = 1;
...
extern void TravelData(BPlusTree T) {
  ...
  while (Tmp != NULL) {
    i = 0;
    while (i < Tmp->KeyNum) printf(" %d", Tmp->Key[i++]);
    Tmp = Tmp->Next;
  }
}
